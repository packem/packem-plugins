...
class PackemMinifyPlugin extends PackemPlugin {
  onEnd(config) {
    ...
    if (!existsSync(config.output)) {
      throw Error(
        "A bundle must be available for `packem-minify-plugin` to work."
      );
    ...
    const outputBundleFileName =
      this.pluginConfig.outputFilename || "bundle.min.js";
    ...
    if (typeof minifiedBundle == "string") {
      ...
      if (minifiedBundle.map) {
        let outputMapFileName =
          outputBundleFileName.replace(/.js$/, ".map") || "bundle.min.map";
  ...
  // @todo Minification in development mode
  // onAfterDevTransform(mod) {
  //   const {
  //     minfyInDevMode = false,
  //     terserOpts = {}
  //   } = this.pluginConfig;

  //   if (!minfyInDevMode) return;

  //   mod.content = terser.minify(bundle, terserOpts)
  // }
}
